---
name: configuring-ops-suite
description: Guides through full ops-suite configuration by detecting kubectl contexts, namespaces, and services, then writing config.yaml. Use when setting up ops-suite for the first time, config.yaml is missing, or the user asks to configure, initialize, or set up ops-suite.
argument-hint: ""
allowed-tools:
  - Bash
  - AskUserQuestion
...
2. > "**{env_name}**: Which namespace runs shared infrastructure (RabbitMQ, PgBouncer, etc.)?"
...
- List up to 4 detected deployments + "I'll type it manually"
...
2. Kubernetes service (e.g. `svc/my-api`)
3. Namespace
4. Port
...
```yaml
# ops-suite configuration — generated by /ops-suite:configuring-ops-suite on {YYYY-MM-DD}
...
Skills now available:
  /ops-suite:service-status     check pod health & restarts
  ...
  /ops-suite:port-forward       connect to cluster svc    [if orchestrator = kubernetes]
